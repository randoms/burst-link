 ...
 * }
 */
void on_remote_data_received(uint8_t *data, const uint8_t *client_id_bin){
    data = "{\"uuid\": \"ecc1025c-25c1-4ab3-a619-679f3daea462\",\"data\": \"I am Chuck Norris!\"}";
    json_error_t *err;
    ...
    const uint8_t *cmd = json_string_value(json_object_get(data_json,"cmd"));
...
void create_remote_socket(const uint8_t *uuid, const uint8_t *client_id_bin){
    write_data_remote(uuid,"CREATE_SOCK","",strlen(""));
}
...
void *on_local_sock_connect(void *msockfd){
    ...
    uint8_t *uuid = get_local_socks_uuid(msocks_list,sockfd);
    uint8_t *target_addr_bin = hex_string_to_bin(target_id);
    create_remote_socket(uuid,target_addr_bin);
